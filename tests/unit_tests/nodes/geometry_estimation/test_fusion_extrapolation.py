...
def test_returns_circle_when_pupil_not_inside_iris(input_polygons: GeometryPolygons, eye_center: EyeCenters) -> None:
    # Make one-sided iris arc very short and elliptical so ellipse estimation will cause pupil to be partially outside iris → all_inside == False
    ...
    ellipse_poly = LSQEllipseFitWithRefinement(dphi=360 / 512).run(input_polygons)
    all_inside = fx._all_points_in_poly(ellipse_poly.pupil_array, ellipse_poly.iris_array)
